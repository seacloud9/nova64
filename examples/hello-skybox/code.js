...
export async function init() {
  ...
  for (let i = 0; i < 45; i++) {
    ...
    const mesh = createSphere(r, 0x554433, [x, y, z], 7,
  ...
  for (const p of planetDefs) {
    createSphere(p.r, p.color, [p.x, p.y, p.z], 20,
      { material: 'standard', roughness: 0.7 });
  }
  ...
  for (let i = 0; i < TOTAL; i++) {
    ...
    const mesh = createCube(2.5, color, [x, y, z],
...
export function update(dt) {
  ...
  for (const c of crystals) {
    ...
    if (dx*dx + dy*dy + dz*dz < 100) {   // ≤ 10 unit radius
      removeMesh(c.mesh);
      c.active = false;
      collected++;
      score += 100;
...
export function draw() {
  const spd = Math.round(
    Math.sqrt(vel.x*vel.x + vel.y*vel.y + vel.z*vel.z) * 10
  );
