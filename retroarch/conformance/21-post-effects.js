...
// exposure, saturation, temperature, vibrance, sharpness, film grain, and HDR target state round-trips.
...
export function init() {
  ...
  nova64.post.setTemperature(0.35);
  nova64.post.setVibrance(0.45);
  ...
  if (Math.abs(s.bloomRadius - 0.35) > 0.01)
    throw new Error('bloom radius mismatch: ' + s.bloomRadius);
  if (Math.abs(s.bloomThreshold - 0.8) > 0.01)
    throw new Error('bloom threshold mismatch: ' + s.bloomThreshold);
  ...
  if (Math.abs(s.temperature - 0.35) > 0.01)
    throw new Error('temperature mismatch: ' + s.temperature);
  if (Math.abs(s.vibrance - 0.45) > 0.01) throw new Error('vibrance mismatch: ' + s.vibrance);
  ...
  if (s2.temperature !== 0) throw new Error('temperature should be 0 after clear');
  if (s2.vibrance !== 0) throw new Error('vibrance should be 0 after clear');
  ...
  nova64.post.setTemperature(-0.25);
  nova64.post.setVibrance(0.55);
...
export function draw() {
  ...
  print('temp=' + s.temperature.toFixed(2), 4, 54, rgba8(255, 190, 140, 255));
  print('fbo=' + s.fboReady, 4, 64, rgba8(180, 180, 180, 255));
}
