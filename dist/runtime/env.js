...
function applyEnv(config) {
  ...
  if (light) {
    ...
    if (light.directional && typeof globalThis.setDirectionalLight === 'function') {
      ...
      globalThis.setDirectionalLight(d.direction || [1, 2, 1], parseColor(d.color ?? 0xffffff), d.intensity ?? 1);
    }
  ...
  if (fx) {
    ...
    if (fx.lowPolyMode && typeof globalThis.enableLowPolyMode === 'function') globalThis.enableLowPolyMode();
  }
...
function renderOverlay() {
  ...
  if (keys.length > 0) {
    ...
    html += '<p style="color:#666;font-size:14px">No cheats defined in this cart\'s env config.</p>';
  }
  ...
  if (_applied) {
    if (_currentLevel) info += `<div style="margin:4px 0">Level: <span style="color:#ff0">${_currentLevel}</span></div>`;
    if (_applied.sky) info += `<div style="margin:4px 0">Sky: <span style="color:#ff0">${_applied.sky.type || 'none'}</span></div>`;
    if (_applied.fog) info += `<div style="margin:4px 0">Fog: <span style="color:#ff0">${_applied.fog.enabled !== false ? `color=0x${(parseColor(_applied.fog.color)).toString(16).padStart(6, '0')} near=${_applied.fog.near} far=${_applied.fog.far}` : 'off'}</span></div>`;
    if (_applied.camera) info += `<div style="margin:4px 0">Camera: <span style="color:#ff0">pos=[${(_applied.camera.position || []).join(',')}] fov=${_applied.camera.fov ?? 'default'}</span></div>`;
    if (_applied.voxel) info += `<div style="margin:4px 0">Voxel: <span style="color:#ff0">rd=${_applied.voxel.renderDistance ?? 4} seed=${_applied.voxel.seed ?? 'random'}</span></div>`;
  } else {
