...
function buildMockGlobal() {
  ...
  // Camera
  for (const fn of ['setCameraPosition', 'setCameraTarget', 'setCameraLookAt', 'setCameraFOV'])
    g[fn] = stub(fn);
  ...
  // Color
  g.rgba8 = (r, g2, b, a) =>
    ((r & 0xff) << 24) | ((g2 & 0xff) << 16) | ((b & 0xff) << 8) | (a & 0xff);
  ...
  // 2D transform
  for (const fn of ['pushMatrix', 'popMatrix', 'translate', 'rotate', 'scale2d', 'resetMatrix'])
    g[fn] = stub(fn);
  ...
  g.grid = (cols, rows, cellW, cellH, padX, padY) => {
    ...
    for (let r = 0; r < rows; r++)
      for (let c = 0; c < cols; c++)
        cells.push({ x: c * (cellW + (padX || 0)), y: r * (cellH + (padY || 0)), col: c, row: r });
    return cells;
  };
  g.frameCount = 0;
  ...
  g.createPool = (max = 100, factory) => {
    ...
    return {
      ...
      get count() {
        let n = 0;
        for (const o of items) if (o._poolAlive) n++;
        return n;
      },
    };
  };
...
export async function runWizardryTests() {
  ...
  runner.test('Wizardry - switchState guards novaStore access', () => {
    ...
    const hasGuard =
      body.includes('novaStore?.') ||
      body.includes('if (novaStore') ||
      body.includes('typeof novaStore') ||
      body.includes('try {');
    assert(
      hasGuard,
      'switchState must guard novaStore access to prevent crashes when runtime has not exposed it yet'
    );
  });
  ...
  runner.test('Wizardry - setCamera(0,0) reset happens after ALL 2D drawing', () => {
    ...
    for (const dc of drawCalls) {
      assert(
        !afterReset.includes(dc),
        `Draw call '${dc}' appears AFTER setCamera(0,0) reset — this means it draws at wrong offset when shake is active`
      );
    }
  });
  ...
  runner.test('Wizardry - drawTitle produces visible output at t=0', () => {
    ...
    assert(
      firstCls === -1,
      'drawTitle should NOT call cls() — that would black out the 3D scene behind the fade-in'
    );
  });
  ...
  runner.test('Wizardry - init() runs without throwing', async () => {
    ...
    try {
      ...
      mod.init();
      assert(
        typeof calls['createStateMachine'] === 'undefined' || true,
        'init should call createStateMachine (handled internally)'
      );
    } finally {
      cleanupGlobals(g);
    }
  ...
  runner.test('Wizardry - Title screen draw produces draw calls', async () => {
    ...
    try {
      ...
      const titleDraws =
        (calls['drawSkyGradient'] || 0) +
        (calls['drawGlowText'] || 0) +
        (calls['printCentered'] || 0);
      assert(
        titleDraws > 0,
        `Title screen should produce draw calls but got 0 (drawSkyGradient=${calls['drawSkyGradient'] || 0}, drawGlowText=${calls['drawGlowText'] || 0}, printCentered=${calls['printCentered'] || 0})`
      );
    } finally {
      cleanupGlobals(g);
    }
