...
export function init() {
  ...
  if (!tryAutoLoadDefaultWAD()) loadWAD();
}
...
export function update(dt) {
  ...
  // Keyboard navigation for menu (works in both browser and Godot)
  if (gameState === 'menu') {
    if (keyp('ArrowUp') || keyp('KeyW')) {
      if (currentMapIdx > 0) currentMapIdx--;
    }
    if (keyp('ArrowDown') || keyp('KeyS')) {
      if (currentMapIdx < mapNames.length - 1) currentMapIdx++;
    }
    if (keyp('Enter') || keyp('Space') || btn('A')) startLevel();
  }
...
function drawMenu() {
  ...
  for (let i = startIdx; i < endIdx; i++) {
    ...
    const title = wadMapTitles[mapNames[i]];
    const label = title ? `${mapNames[i]}: ${title}` : mapNames[i];
    printCentered(`${prefix}${label}`, W / 2, y, color);
  }
