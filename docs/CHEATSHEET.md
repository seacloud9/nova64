...
```js
let cube;
export function init() { cube = createCube(1, 0x00aaff, [0,0,-5]); }
export function update(dt) { rotateMesh(cube, 0, dt, 0); }
```
...
**Sunset gradient sky**
```js
export function init() { createGradientSkybox(0x1a6aa8, 0xf4a460); }
```
