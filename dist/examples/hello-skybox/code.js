...
export async function init() {
  ...
  for (let i = 0; i < 45; i++) {
    ...
    const mesh = nova64.scene.createSphere(r, 0x554433, [x, y, z], 7, { material: 'standard', roughness: 0.95 });
    asteroids.push({
      mesh,
      x,
      y,
  ...
  for (const p of planetDefs) {
    nova64.scene.createSphere(p.r, p.color, [p.x, p.y, p.z], 20, { material: 'standard', roughness: 0.7 });
  }
  ...
  for (let i = 0; i < TOTAL; i++) {
    ...
    const mesh = nova64.scene.createCube(2.5, color, [x, y, z], { material: 'emissive', emissive: color });
...
export function update(dt) {
  ...
  // Pitch  (clamp to ±75°)
  if (nova64.input.key('KeyQ') || nova64.input.key('ArrowUp')) pitch = Math.min(pitch + PITCH_SPD * dt, 75);
  if (nova64.input.key('KeyE') || nova64.input.key('ArrowDown')) pitch = Math.max(pitch - PITCH_SPD * dt, -75);
...
export function draw() {
  ...
  // Top-left HUD
  nova64.draw.print(`CRYSTALS  ${score.toString().padStart(6, '0')}`, 16, 16, nova64.draw.rgba8(0, 255, 200, 255));
  nova64.draw.print(`SPEED     ${spd.toString().padStart(3)} u/s`, 16, 36, nova64.draw.rgba8(160, 200, 255, 210));
  nova64.draw.print(`REMAINING ${collected} / ${TOTAL}`, 16, 56, nova64.draw.rgba8(255, 220, 80, 200));

  // Controls hint — fades out after 8 s
  if (time < 8) {
    const a = Math.min(255, Math.floor((8 - time) * 50));
    nova64.draw.print('W / SPACE — Thrust     S — Brake', 320, 328, nova64.draw.rgba8(200, 200, 200, a));
    nova64.draw.print('A / D     — Turn       Q / E — Pitch', 320, 346, nova64.draw.rgba8(200, 200, 200, a));
  }
