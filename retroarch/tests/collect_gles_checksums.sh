#!/usr/bin/env bash
# Collect GLES hardware checksums for all 3D visual carts.
# Usage: bash retroarch/tests/collect_gles_checksums.sh
set -uo pipefail
...
for cart in "${CARTS[@]}"; do
  ...
  [[ -f "${js}" ]] || continue
  result=$(timeout 30 "${HARNESS}" "${CORE}" "${js}" --gles 2>/dev/null) || true
