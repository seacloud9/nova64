...
export function init() {
  // ── Palette ──────────────────────────────────────────────────────────────
  palette = nova64.tween.createColorPool(
    [
      nova64.draw.rgba8(255, 60, 120),
      nova64.draw.rgba8(60, 200, 255),
      nova64.draw.rgba8(255, 200, 60),
      nova64.draw.rgba8(120, 255, 120),
      nova64.draw.rgba8(200, 80, 255),
      nova64.draw.rgba8(255, 140, 60),
      nova64.draw.rgba8(60, 255, 220),
      nova64.draw.rgba8(255, 255, 60),
    ],
    'sequential'
  );

  // ── Section 1 — Circle Layout + ColorPool ────────────────────────────────
  const circLayout = nova64.tween.createCircleLayout({ count: 12, radius: 5, plane: 'xz', cy: 0 });
  circLayout.each(({ x, y, z }, i) => {
    const color = 0x4488ff + i * 0x100800;
    const cube = nova64.scene.createCube(0.6, color, [x, y, z]);
    s1Cubes.push({ mesh: cube, x, y, z, i });
    // Oscillator per cube — vertical bounce offset by index
    const osc = nova64.tween.createOscillator({
      waveform: 'sin',
      min: -1.5,
      max: 1.5,
      speed: 0.6,
  ...
  // ── Section 2 — Grid Layout + Oscillators ────────────────────────────────
  const gridLayout = nova64.tween.createGridLayout({
    cols: 4,
    rows: 4,
    spacingX: 2.2,
    spacingY: 2.2,
  ...
  gridLayout.each(({ x, y, z }, i) => {
    const cube = nova64.scene.createCube(0.8, 0x223344, [x, y - 4, z]);
  ...
  // ── Section 3 — Camera Tween ──────────────────────────────────────────────
  s3Tween = nova64.tween.createTween({
    from: [0, 8, 16],
    to: [8, 3, 6],
    duration: 4,
    ease: 'easeInOutCubic',
    ...
    onUpdate([x, y, z]) {
      ...
      nova64.camera.setCameraTarget(0, 0, -4);
    },
  });
  ...
  // ── Section 5 — Swarm ─────────────────────────────────────────────────────
  s5Swarm = nova64.tween.createHSwarm({
    count: 30,
    separation: 1.8,
    alignment: 0.8,
    cohesion: 0.6,
  ...
  for (let i = 0; i < s5Swarm.agents.length; i++) {
    const mesh = nova64.scene.createCube(0.25, 0x00eeff, [0, 0, -10]);
    s5Meshes.push(mesh);
  }
  ...
  s5GoalTimer = nova64.tween.createTimeTrigger({
    interval: 4,
    repeat: true,
    callback() {
      s5Swarm.clearGoals();
  ...
  s6PathLayout = nova64.tween.createPathLayout({
    mode: 'bezier',
    count: 8,
    points: [
      { x: -7, y: -2, z: -10 },
  ...
  s6PathLayout.each(({ x, y, z }, i) => {
    const mesh = nova64.scene.createSphere(0.3, 0x334466, [x, y, z]);
    s6PathMeshes.push({ mesh, x, y, z });
  });

  s6RiderMesh = nova64.scene.createSphere(0.6, 0xffaa00, [0, 0, -10]);

  s6RiderTween = nova64.tween.createTween({
    from: 0,
    to: 1,
    duration: 6,
    ease: 'easeInOutCubic',
    ...
    onUpdate(t) {
      ...
      s6RiderPos.y = pos.y;
      s6RiderPos.z = pos.z;
      nova64.scene.setPosition(s6RiderMesh, pos.x, pos.y, pos.z);
    },
  });
  s6RiderTween.start();

  // ── Section 7 — Sphere Layout + ColorPool Shuffle ────────────────────────
  s7ColorPool = nova64.tween.createColorPool(
    [
      nova64.draw.rgba8(255, 60, 100),
      nova64.draw.rgba8(255, 140, 40),
      nova64.draw.rgba8(255, 220, 40),
      nova64.draw.rgba8(80, 220, 60),
      nova64.draw.rgba8(40, 200, 255),
      nova64.draw.rgba8(120, 80, 255),
      nova64.draw.rgba8(255, 80, 200),
      nova64.draw.rgba8(200, 255, 100),
    ],
    'shuffle'
  );

  const sphereLayout = nova64.tween.createSphereLayout({ count: 24, radius: 4.5 });
  sphereLayout.each(({ x, y, z }) => {
    const mesh = nova64.scene.createSphere(0.35, 0x224466, [x, y - 2, z - 10]);
    s7Meshes.push({ mesh, ox: x, oy: y - 2, oz: z - 10 });
  });

  // Rotate color assignments every 1.5s
  s7SwapTimer = nova64.tween.createTimeTrigger({
    interval: 1.5,
    repeat: true,
    callback() {
      s7ColorPool.shuffle();
  ...
  s8Pool = nova64.tween.createHPool({
    build() {
      return { mesh: nova64.scene.createCube(0.5, 0x334455, [0, 0, -8]), tween: null };
    },
    onCreate(obj) {
      nova64.scene.setMeshVisible(obj.mesh, false);
    },
    onRequest(obj) {
      const color = HUE_COLORS[_s8HueIdx % HUE_COLORS.length];
      _s8HueIdx++;
      // Quick tween: scale 0.01 → 1 with bounce
      obj.tween = nova64.tween.createTween({
        from: [0.01, (Math.random() - 0.5) * 8, (Math.random() - 0.5) * 4, -8],
        to: [1, (Math.random() - 0.5) * 8, (Math.random() - 0.5) * 4, -8],
        duration: 1.2,
        ease: 'easeOutBounce',
        onUpdate([sc, x, y, z]) {
          nova64.scene.setScale(obj.mesh, sc, sc, sc);
          nova64.scene.setPosition(obj.mesh, x, y, z);
        },
      });
      obj.tween.start();
      obj.color = color;
      nova64.scene.setMeshVisible(obj.mesh, true);
    },
    onRelease(obj) {
      nova64.scene.setMeshVisible(obj.mesh, false);
    },
  ...
  s8RandomTrigger = nova64.tween.createRandomTrigger({
    chance: 0.04,
    callback() {
      s8Pool.request();
    },
  ...
  s8ReleaseTrigger = nova64.tween.createTimeTrigger({
    interval: 2.5,
    repeat: true,
    callback() {
  ...
  s9OscX = nova64.tween.createOscillator({ waveform: 'sin', min: -5, max: 5, speed: 0.3 });
  s9OscY = nova64.tween.createOscillator({ waveform: 'sin', min: -3, max: 3, speed: 0.4 });
  s9OscZ = nova64.tween.createOscillator({ waveform: 'cos', min: -4, max: 4, speed: 0.5 });
  s9OscScale = nova64.tween.createOscillator({ waveform: 'sin', min: 0.15, max: 0.55, speed: 0.7 });
  ...
  for (let i = 0; i < CASCADE_COUNT; i++) {
    ...
    const tw = nova64.tween.createTween({
      ...
      loop: 'none',
      onUpdate(sc) {
        nova64.scene.setScale(mesh, sc, sc, sc);
      },
    });
    s10Tweens.push(tw);
  }

  s10CascadeTimer = nova64.tween.createTimeTrigger({
    interval: 0.25,
    repeat: true,
    callback() {
  ...
  // ── Section 11 — Sawtooth Traveling Wave ────────────────────────────────────
  s11ColorPool = nova64.tween.createColorPool(
    [
      nova64.draw.rgba8(255, 60, 120),
      nova64.draw.rgba8(60, 200, 255),
      nova64.draw.rgba8(255, 220, 60),
      nova64.draw.rgba8(120, 255, 120),
      nova64.draw.rgba8(200, 80, 255),
      nova64.draw.rgba8(255, 140, 60),
    ],
  ...
  for (let i = 0; i < WAVE_COUNT; i++) {
    ...
    const mesh = nova64.scene.createCube(0.65, color, [(i - WAVE_COUNT / 2 + 0.5) * 1.5, 0, -8]);
    s11Cubes.push(mesh);
...
function _setSection(s) {
  ...
  s1Cubes.forEach(({ mesh }) => mesh && nova64.scene.setMeshVisible(mesh, false));
  s2Cubes.forEach(({ mesh }) => mesh && nova64.scene.setMeshVisible(mesh, false));
  if (s4SphereA) nova64.scene.setMeshVisible(s4SphereA, false);
  if (s4SphereB) nova64.scene.setMeshVisible(s4SphereB, false);
  s5Meshes.forEach(m => m && nova64.scene.setMeshVisible(m, false));
  s6PathMeshes.forEach(({ mesh }) => nova64.scene.setMeshVisible(mesh, false));
  if (s6RiderMesh) nova64.scene.setMeshVisible(s6RiderMesh, false);
  s7Meshes.forEach(({ mesh }) => nova64.scene.setMeshVisible(mesh, false));
  s8Pool.active.forEach(o => nova64.scene.setMeshVisible(o.mesh, false));
  s8Pool.releaseAll();
  s9TrailMeshes.forEach(m => nova64.scene.setMeshVisible(m, false));
  s10Cubes.forEach(m => nova64.scene.setMeshVisible(m, false));
  s11Cubes.forEach(m => nova64.scene.setMeshVisible(m, false));

  if (s === 0) {
    s1Cubes.forEach(({ mesh }) => nova64.scene.setMeshVisible(mesh, true));
    nova64.camera.setCameraPosition(0, 5, 14);
    nova64.camera.setCameraTarget(0, 0, 0);
  } else if (s === 1) {
    s2Cubes.forEach(({ mesh }) => nova64.scene.setMeshVisible(mesh, true));
    nova64.camera.setCameraPosition(-2, 6, 10);
    nova64.camera.setCameraTarget(0, -2, -8);
  } else if (s === 2) {
    s3Tween.start();
    s2Cubes.forEach(({ mesh }) => nova64.scene.setMeshVisible(mesh, true));
  } else if (s === 3) {
    nova64.scene.setMeshVisible(s4SphereA, true);
    nova64.scene.setMeshVisible(s4SphereB, true);
    nova64.camera.setCameraPosition(0, 6, 2);
    nova64.camera.setCameraTarget(0, 0, -8);
  } else if (s === 4) {
    s5Meshes.forEach(m => nova64.scene.setMeshVisible(m, true));
    nova64.camera.setCameraPosition(0, 10, 5);
    nova64.camera.setCameraTarget(0, 0, -10);
  } else if (s === 5) {
    s6PathMeshes.forEach(({ mesh }) => nova64.scene.setMeshVisible(mesh, true));
    nova64.scene.setMeshVisible(s6RiderMesh, true);
    s6RiderTween.start();
    nova64.camera.setCameraPosition(0, 6, 4);
    nova64.camera.setCameraTarget(0, 0, -10);
  } else if (s === 6) {
    s7Meshes.forEach(({ mesh }) => nova64.scene.setMeshVisible(mesh, true));
    s7RotOsc.reset();
    nova64.camera.setCameraPosition(0, 4, 10);
    nova64.camera.setCameraTarget(0, -2, -10);
  } else if (s === 7) {
    nova64.camera.setCameraPosition(0, 5, 8);
    nova64.camera.setCameraTarget(0, 0, -8);
  } else if (s === 8) {
    s9TrailMeshes.forEach(m => nova64.scene.setMeshVisible(m, true));
    s9History.forEach(h => {
      h.x = 0;
      h.y = 0;
      h.z = -8;
    ...
    s10Cubes.forEach(m => {
      nova64.scene.setMeshVisible(m, true);
      nova64.scene.setScale(m, 0.4, 0.4, 0.4);
    });
    s10Index = 0;
    nova64.camera.setCameraPosition(0, 4, 10);
    nova64.camera.setCameraTarget(0, 0, -8);
  } else if (s === 10) {
    s11Cubes.forEach(m => nova64.scene.setMeshVisible(m, true));
    nova64.camera.setCameraPosition(0, 5, 14);
    nova64.camera.setCameraTarget(0, 0, -8);
  }
}
...
export function update(dt) {
  ...
  if (nova64.input.keyp('Space')) _nextSection();
  ...
  if (section === 0) {
    for (let i = 0; i < s1Cubes.length; i++) {
      ...
      nova64.scene.setPosition(mesh, x, yVal, z);
      nova64.scene.rotateMesh(mesh, dt * 0.5, dt * 0.8, 0);
    }
  }
  ...
  if (section === 1) {
    for (let i = 0; i < s2Cubes.length; i++) {
      ...
      nova64.scene.setScale(mesh, sc, sc, sc);
      nova64.scene.rotateMesh(mesh, 0, dt * (0.5 + (i % 4) * 0.2), 0);
    }
  }
  ...
  if (section === 2) {
    ...
    for (let i = 0; i < s2Cubes.length; i++) {
      ...
      const sc = 0.5 + (s2Oscillators[i].value + 1) * 0.4;
      nova64.scene.setScale(mesh, sc, sc, sc);
      nova64.scene.rotateMesh(mesh, 0, dt * 0.6, 0);
    }
  }
  ...
  if (section === 3) {
    ...
    nova64.scene.setPosition(s4SphereA, s4APos.x, s4APos.y, s4APos.z);
    nova64.scene.setPosition(s4SphereB, s4BPos.x, s4BPos.y, s4BPos.z);
    s4Proximity.tick(dt);
  }
  ...
  if (section === 4) {
    ...
    for (let i = 0; i < s5Swarm.agents.length; i++) {
      ...
      nova64.scene.setPosition(s5Meshes[i], a.x, a.y, a.z);
      nova64.scene.rotateMesh(s5Meshes[i], dt, dt * 1.3, 0);
    }
  }
  ...
  if (section === 5) {
    ...
    for (let i = 0; i < s6PathMeshes.length; i++) {
      ...
      nova64.scene.setScale(s6PathMeshes[i].mesh, sc, sc, sc);
    }
    nova64.scene.rotateMesh(s6RiderMesh, dt * 0.4, dt * 1.2, dt * 0.7);
  }
  ...
  if (section === 6) {
    ...
    for (let i = 0; i < s7Meshes.length; i++) {
      ...
      nova64.scene.setPosition(mesh, rx, oy, rz - 10);
      // Pulse scale per sphere
      const sc = 0.7 + Math.sin(time * 1.8 + i * 0.4) * 0.3;
      nova64.scene.setScale(mesh, sc, sc, sc);
      nova64.scene.rotateMesh(mesh, dt * 0.3, dt * 0.5, 0);
    }
  }
  ...
  if (section === 7) {
    ...
    for (const obj of s8Pool.active) {
      ...
      nova64.scene.rotateMesh(obj.mesh, dt * 0.6, dt, 0);
    }
  }
  ...
  if (section === 8) {
    ...
    for (let i = 0; i < s9TrailMeshes.length; i++) {
      ...
      nova64.scene.setPosition(s9TrailMeshes[i], h.x, h.y, h.z);
      nova64.scene.setScale(s9TrailMeshes[i], sc, sc, sc);
    }
  }
  ...
  if (section === 10) {
    for (let i = 0; i < WAVE_COUNT; i++) {
      ...
      nova64.scene.setPosition(s11Cubes[i], x, yVal, z);
      // Scale slightly with wave height
      const sc = 0.5 + Math.abs(yVal) * 0.18;
      nova64.scene.setScale(s11Cubes[i], sc, sc, sc);
      nova64.scene.rotateMesh(s11Cubes[i], dt * 0.3, dt * 0.5, 0);
    }
  }
}
...
export function draw() {
  ...
  const cWhite = nova64.draw.rgba8(255, 255, 255);
  const cCyan = nova64.draw.rgba8(0, 238, 255);
  const cGrey = nova64.draw.rgba8(136, 153, 204);
  ...
  nova64.draw.drawPanel(4, 4, 220, 14, {
    bgColor: nova64.draw.rgba8(0, 0, 12, 210),
    borderLight: cCyan,
    borderDark: nova64.draw.rgba8(0, 60, 80),
  });
  nova64.draw.print(`HYPE: ${labels[section]}`, 8, 8, cWhite);

  // Section counter badge
  nova64.draw.drawPanel(W - 38, 4, 34, 14, {
    bgColor: nova64.draw.rgba8(0, 0, 12, 210),
    borderLight: cGrey,
    borderDark: nova64.draw.rgba8(30, 40, 70),
  });
  nova64.draw.print(`${section + 1}/11`, W - 34, 8, cGrey);

  // Nav hint
  nova64.draw.print('SPACE: next', W - 74, H - 10, nova64.draw.rgba8(80, 90, 110));
  ...
  const panel = col => ({
    bgColor: nova64.draw.rgba8(0, 0, 12, 210),
    borderLight: col,
    borderDark: nova64.draw.rgba8(0, 0, 0),
  });

  if (section === 3) {
    const dist = s4Proximity?.distance?.toFixed(1) ?? '0.0';
    const col = s4Highlight ? nova64.draw.rgba8(255, 80, 60) : nova64.draw.rgba8(100, 220, 180);
    nova64.draw.drawPanel(4, H - 24, 170, 14, panel(col));
    nova64.draw.print(`Dist: ${dist}  ${s4Highlight ? 'CONTACT!' : ''}`, 8, H - 20, col);
  }

  if (section === 4) {
    nova64.draw.drawPanel(4, H - 24, 210, 14, panel(cCyan));
    nova64.draw.print(
  ...
  if (section === 7) {
    nova64.draw.drawPanel(4, H - 24, 215, 14, panel(nova64.draw.rgba8(136, 255, 68)));
    nova64.draw.print(
      `Pool active: ${s8Pool?.active?.length ?? 0}  waiting: ${s8Pool?.available ?? 0}`,
      8,
      H - 20,
      nova64.draw.rgba8(136, 255, 68)
    );
  }
  ...
  if (section === 9) {
    nova64.draw.drawPanel(4, H - 24, 215, 14, panel(nova64.draw.rgba8(255, 204, 68)));
    nova64.draw.print(
      `Cascade idx: ${s10Index % CASCADE_COUNT}/${CASCADE_COUNT}  interval: 0.25s`,
      8,
      H - 20,
      nova64.draw.rgba8(255, 204, 68)
    );
  }
