...
import { processCartCode, getNovaBaseUrl, fetchCartCode } from '../utils/cartCode';
...
export function GameStudio() {
  ...
  const sendCodeToIframe = (rawCode: string) => {
    ...
    if (iframeReadyRef.current && iframeRef.current?.contentWindow) {
      iframeRef.current.contentWindow.postMessage(
        { type: 'EXECUTE_CODE', code: processed },
        getNovaBaseUrl()
      );
      pendingCodeRef.current = null;
      return true;
  ...
  const loadDemo = async (key: string) => {
    ...
    try {
      ...
      if ('code' in demo) {
        ...
        gameCode = await fetchCartCode((demo as DemoWithPath).path);
      }
      setCode(gameCode);
      setFileName(`${key}.js`);
      setIsDemo(true);
  ...
  const forkDemo = () => {
    ...
    setEditorReadOnly(false);
    setOutput([
      `\uD83D\uDD00 Forked \u201C${demoName ?? fileName}\u201D \u2192 \u201C${base}\u201D (now editable)`,
    ]);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          📂 My Carts{savedCarts.length > 0 ? ` (${savedCarts.length})` : ''}
        </button>
        ...
        {/* New */}
        <button onClick={newGame} style={btn('linear-gradient(180deg, #606060 0%, #404040 100%)')}>
          📄 New
        </button>
          ...
          {savedCarts.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {savedCarts.map(name => (
                <button
                  key={name}
                  onClick={() => loadSavedCart(name)}
                  style={{
                    padding: '7px 13px',
                    background:
                      name === fileName && !isDemo
