...
Nova64 now has an experimental NOA probe seam at:

- `runtime/backends/babylon/noa-prototype.js`

This is intentionally not a full runtime swap. The current Babylon voxel path still lives in:
...
The probe currently does one thing safely: it attempts to load `noa-engine` and reports whether the module is available, without handing runtime ownership to NOA.
...
`active` should stay `false` until a real adapter is implemented.
...
5. If NOA is vendored later, test build stability first because of the Babylon 6 vs 7 compatibility risk.
