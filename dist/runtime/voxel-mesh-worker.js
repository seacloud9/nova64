...
function meshChunk(data) {
  const {
    blocks, skyLight, blockLight, fluidLevel,
    baseX, baseZ,
    enableAO, enableLighting, atlasEnabled, dayTimeFactor,
  } = data;
  ...
  const opaqueVerts = [], opaqueNorms = [], opaqueColors = [], opaqueUvs = [], opaqueUv2s = [], opaqueIdx = [];
  let opaqueVCount = 0;
  const transVerts = [], transNorms = [], transColors = [], transUvs = [], transUv2s = [], transIdx = [];
  let transVCount = 0;
    ...
    for (let d = 0; d < sliceMax; d++) {
      ...
      for (let vi = 0; vi < vMax; vi++) {
        for (let ui = 0; ui < uMax; ui++) {
          ...
          let x = 0, y = 0, z = 0;
          if (fd.axis === 0) x = d; else if (fd.axis === 1) y = d; else z = d;
          if (fd.u === 0) x = ui; else if (fd.u === 1) y = ui; else z = ui;
          if (fd.v === 0) x = vi; else if (fd.v === 1) y = vi; else z = vi;

          const blockType = pBlock(x, y, z);
          const mIdx = ui + vi * uMax;

          if (blockType === 0) { mask[mIdx] = 0; maskTrans[mIdx] = 0; continue; }
          ...
          if (blockIsTransparent) {
            ...
            else if (solidArr[neighborId] === 1 && transparentArr[neighborId] === 0) showFace = false;
            else showFace = true;
          } else {
            showFace = solidArr[neighborId] === 0 || transparentArr[neighborId] === 1;
          }

          if (!showFace) { mask[mIdx] = 0; maskTrans[mIdx] = 0; continue; }

          // AO
          let aoKey = 0;
          if (enableAO) {
            for (let ci = 0; ci < 4; ci++) {
              const s1 = isOpaqueForAO(x + aoOff[ci][0][0], y + aoOff[ci][0][1], z + aoOff[ci][0][2]);
              const s2 = isOpaqueForAO(x + aoOff[ci][1][0], y + aoOff[ci][1][1], z + aoOff[ci][1][2]);
              const cn = isOpaqueForAO(x + aoOff[ci][2][0], y + aoOff[ci][2][1], z + aoOff[ci][2][2]);
          ...
          mask[mIdx] = (blockType & 0xff) | ((aoKey & 0xff) << 8) | ((faceLight & 0xf) << 16) | (1 << 24);
      ...
      for (let vi = 0; vi < vMax; vi++) {
        for (let ui = 0; ui < uMax;) {
          const mIdx = ui + vi * uMax;
          const key = mask[mIdx];
          if (key === 0) { ui++; continue; }
          const trans = maskTrans[mIdx];
          const flLvl = maskFluidLvl[mIdx];

          // Extend width
          let w = 1;
          while (ui + w < uMax && mask[ui + w + vi * uMax] === key &&
                 maskTrans[ui + w + vi * uMax] === trans &&
                 maskFluidLvl[ui + w + vi * uMax] === flLvl) w++;
          ...
          if (fd.axis === 0) { c0x = c1x = c2x = c3x = slicePos; }
          else if (fd.axis === 1) { c0y = c1y = c2y = c3y = slicePos; }
          else { c0z = c1z = c2z = c3z = slicePos; }
          if (fd.u === 0) { c0x = ui; c1x = ui + w; c2x = ui + w; c3x = ui; }
          else if (fd.u === 1) { c0y = ui; c1y = ui + w; c2y = ui + w; c3y = ui; }
          else { c0z = ui; c1z = ui + w; c2z = ui + w; c3z = ui; }
          if (fd.v === 0) { c0x = vi; c1x = vi; c2x = vi + h; c3x = vi + h; }
          else if (fd.v === 1) { c0y = vi; c1y = vi; c2y = vi + h; c3y = vi + h; }
          ...
          const fluidYOff = (faceIdx === 4 && flLvl > 0 && flLvl < FLUID_MAX)
...
self.onmessage = function (e) {
  ...
  if (msg.type === 'mesh') {
    ...
    if (result.hasOpaque) {
      transfers.push(
        result.opaqueVerts.buffer, result.opaqueNorms.buffer,
        result.opaqueColors.buffer, result.opaqueUvs.buffer,
        result.opaqueUv2s.buffer, result.opaqueIdx.buffer
      );
    }
    if (result.hasTrans) {
      transfers.push(
        result.transVerts.buffer, result.transNorms.buffer,
        result.transColors.buffer, result.transUvs.buffer,
        result.transUv2s.buffer, result.transIdx.buffer
      );
    }
    self.postMessage({
      type: 'result',
