...
(async () => {
  ...
  let gotEvent = null;
  r2.onMessage('event', e => {
    gotEvent = e;
  });
  r1.send('chat', { text: 'hi' });
  await waitFor(
    () => gotEvent && gotEvent.type === 'chat' && gotEvent.msg.text === 'hi',
    3000,
    'relay event'
  );

  // alice renames live (/nick → setName); bob's replicated copy updates.
  r1.send('setName', { name: 'alice2' });
  await waitFor(
    () => {
      const a = r2.state.players.get(r1.sessionId);
      return a && a.name === 'alice2';
    },
    3000,
    'setName live rename synced'
  );
  assert.strictEqual(r2.state.players.get(r1.sessionId).name, 'alice2', 'live rename synced');

  console.log(
    `PASS: 2 clients synced (players=${r2.state.players.size}), move replicated, name synced, /nick rename synced, relay delivered`
  );
