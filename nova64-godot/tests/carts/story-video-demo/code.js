...
export function draw() {
  ...
  if (screen === 'story') {
    if (hasStoryHelper() && typeof nova64.story._draw === 'function' && nova64.story._draw()) return;
    drawLocalStory();
    return;
  }
  if (screen === 'video') {
    if (hasVideoHelper() && typeof nova64.video._draw === 'function' && nova64.video._draw()) return;
    drawLocalVideo();
    return;
  }
...
function drawVideoError() {
  ...
  nova64.draw.print('Expected assets/video/sample.ogv on native hosts.', 32, H / 2 + 20, 0xffaaaa, 1);
}
