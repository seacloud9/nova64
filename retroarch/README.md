...
For a Windows RetroArch DLL from WSL:
...
The demoscene visual comparator captures browser and RetroArch GLES frames,
diffs them, and writes `retroarch/build/demoscene-parity/report.json`:

```bash
NOVA64_GLES_TESTS=1 pnpm run retroarch:visual:demoscene
```
...
drawLightning(x0, y0, x1, y1, segs, color)
drawLightning(x0, y0, x1, y1, color [, glowColor [, opts]])
...
printShadowTight(text, x, y, color, shadowColor [, dx [, dy [, align]]])
printOutlineTight(text, x, y, color, outlineColor [, align])
printRainbowTight(text, x, y, color)
printWaveTight(text, x, y, color [, amplitude [, speed [, phase]]])
printFlashTight(text, x, y, colorA [, colorB [, speed]])
printShakeTight(text, x, y, color [, amount [, seed]])
printGradientTight(text, x, y, colorA, colorB)
